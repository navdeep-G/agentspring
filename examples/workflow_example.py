...
async def extract_keywords(text: str, top_n: int = 5) -> dict:
    ...
    return {"keywords": keywords, "total_keywords": len(word_counts)}
...
class AnalysisAgent(Agent):
    ...
    def __init__(self, name: str = "AnalysisAgent"):
        super().__init__(
            name=name,
            description="Analyzes text and determines which tools to use",
        )
        ...
        try:
            ...
            try:
                ...
                if not isinstance(plan, dict) or "steps" not in plan:
                    raise ValueError(
                        "Invalid response format: missing 'steps' field"
                    )

                results = []
                for step in plan.get("steps", []):
                    if not all(
                        k in step for k in ["tool", "parameters", "reasoning"]
                    ):
                    ...
                    try:
                        ...
                        print(
                            f"[DEBUG] Calling {tool_name} with params: {params}"
                        )
                        ...
                        error_msg = str(e)
                        print(
                            f"[ERROR] Error executing {tool_name}: {error_msg}"
                        )
                        results.append(
                            {
                                "tool": tool_name,
...
async def create_workflow(input_text: str) -> Workflow:
    ...
    workflow.add_agent_node(
        ...
        ),  # Pass the input text directly as the goal
    )
...
async def main():
    ...
    print(
        "I can analyze text for sentiment, generate summaries, and extract keywords."
    )
            ...
            if agent_node and agent_node.status == NodeStatus.COMPLETED:
                ...
                for tool_result in results:
                    ...
                    if tool_name == "analyze_sentiment":
                        print(
                            f"Sentiment: {result_data.get('sentiment', 'N/A')}"
                        )
                        print(f"Score: {result_data.get('score', 'N/A')}")
                        print(
                            f"Analysis: {result_data.get('analysis', 'N/A')}"
                        )

                    elif tool_name == "summarize_text":
                        print(f"Summary: {result_data.get('summary', 'N/A')}")
                        print(
                            f"Sentences: {result_data.get('sentence_count', 0)}"
                        )
                        ...
                        print(
                            f"Total unique keywords: {result_data.get('total_keywords', 0)}"
                        )
                ...
                if agent_node and agent_node.result:
                    print(
                        f"Error: {agent_node.result.get('message', 'Unknown error')}"
                    )
