...
def test_sync_complaint_analysis():
    ...
    for i, complaint in enumerate(test_complaints, 1):
        logger.info(f"Testing sync complaint {i}: {complaint['message'][:50]}...")
...
def test_async_complaint_analysis():
    ...
    task_ids = []
    for i, complaint in enumerate(test_complaints, 1):
        logger.info(f"Submitting async complaint {i}: {complaint['message'][:50]}...")
    ...
    for task_id in task_ids:
        ...
        while attempt < max_attempts:
            ...
            logger.info(f"Task {task_id} status: {status_result.get('status', 'unknown')}")
            
            if status_result.get("status") == "completed":
                logger.info(f"Task {task_id} completed: {status_result.get('result', {})}")
                break
            elif status_result.get("status") == "failed":
                logger.error(f"Task {task_id} failed: {status_result.get('error', 'Unknown error')}")
                break
...
def test_edge_cases():
    ...
    result = make_request("/analyze", "POST", {
        "customer_id": "CUST007",
        "message": ""
    })
...
def test_concurrent_requests():
    ...
    logger.info(f"Concurrent requests completed: {len(concurrent_results)} results")
