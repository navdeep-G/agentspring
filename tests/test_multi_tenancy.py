...
def test_get_tenant_by_api_key_and_id(monkeypatch):
    ...
    # Found in Redis
    t = TenantConfig(tenant_id="t3", name="T3", api_key="k3", routing_rules={}, llm_model="m", max_requests_per_hour=10, features_enabled=[], created_at=datetime.now(), updated_at=datetime.now(), active=True)
    mock_redis.get.return_value = t.model_dump_json().encode()
...
def test_create_tenant_errors(monkeypatch):
    ...
    monkeypatch.setattr("agentspring.multi_tenancy.redis_client", mock_redis)
    manager = TenantManager()
    t = TenantConfig(tenant_id="t4", name="T4", api_key="k4", routing_rules={}, llm_model="m", max_requests_per_hour=10, features_enabled=[], created_at=datetime.now(), updated_at=datetime.now(), active=True)
    monkeypatch.setattr(manager, "get_tenant_by_id", lambda tid: t)
...
def test_check_rate_limit(monkeypatch):
    ...
    monkeypatch.setattr("agentspring.multi_tenancy.redis_client", mock_redis)
    manager = TenantManager()
    t = TenantConfig(tenant_id="t5", name="T5", api_key="k5", routing_rules={}, llm_model="m", max_requests_per_hour=1, features_enabled=[], created_at=datetime.now(), updated_at=datetime.now(), active=True)
    monkeypatch.setattr(manager, "get_tenant_by_id", lambda tid: t)
    mock_redis.get.return_value = None
...
def test_get_tenant_routing_rules(monkeypatch):
    ...
    monkeypatch.setattr("agentspring.multi_tenancy.redis_client", mock_redis)
    manager = TenantManager()
    t = TenantConfig(tenant_id="t6", name="T6", api_key="k6", routing_rules={"foo": "bar"}, llm_model="m", max_requests_per_hour=1, features_enabled=[], created_at=datetime.now(), updated_at=datetime.now(), active=True)
    monkeypatch.setattr(manager, "get_tenant_by_id", lambda tid: t)
    assert manager.get_tenant_routing_rules("t6") == {"foo": "bar"}
    t.routing_rules = {}
    assert manager.get_tenant_routing_rules("t6") == {}
    monkeypatch.setattr(manager, "get_tenant_by_id", lambda tid: (_ for _ in ()).throw(Exception()))
