...
def get_task(task_id: str):
    return {
        ...
        "result": {
            ...
            "classification": "Async Test Classification",
        },
    }
...
def patch_redis():
    if agent.async_task_manager:
        ...
        agent.async_task_manager.celery_app.backend.client.ping.return_value = (
            True
        )
...
def test_sync_analysis():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"   Summary: {data.get('data', {}).get('summary', 'N/A')[:50]}..."
            )
            print(
                f"   Category: {data.get('data', {}).get('category', 'N/A')}"
            )
            print(
                f"   Priority: {data.get('data', {}).get('priority', 'N/A')}"
            )
            assert True
        else:
            print(
                f"❌ Sync analysis failed: {response.status_code}, body: {response.text}"
            )
            raise AssertionError()
    except Exception as e:
...
def test_async_analysis():
    ...
    try:
        ...
        if response.status_code != 200:
            print(
                f"❌ Async submission failed: {response.status_code}, body: {response.text}"
            )
            raise AssertionError()
        ...
        for attempt in range(max_attempts):
            ...
            if status_response.status_code == 200:
                ...
                if status == "completed":
                    ...
                    print(
                        f"❌ Async analysis failed: {status_data.get('error', 'Unknown error')}"
                    )
                    raise AssertionError()
                else:
                    print(
                ...
                print(
                    f"❌ Status check failed: {status_response.status_code}, body: {status_response.text}"
                )
                raise AssertionError()
...
def main():
    ...
    if passed == total:
        print(
            "🎉 All tests passed! Your Enterprise Agent API is working correctly."
        )
        print("\n🌐 Access your API:")
