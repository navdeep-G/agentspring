...
class AsyncTaskPoller:
    ...
    def poll_with_exponential_backoff(
        self, task_id: str, max_attempts: int = 30
    ) -> Dict[str, Any]:
        """
        ...
        for attempt in range(max_attempts):
            ...
            print(
                f"   Attempt {attempt + 1}/{max_attempts}: {status['status']} (waiting {wait_time}s)"
            )
    ...
    def poll_with_progress_tracking(
        self, task_id: str, max_attempts: int = 30
    ) -> Dict[str, Any]:
        """
        ...
        for attempt in range(max_attempts):
            ...
            if "meta" in status:
                ...
                if "current" in meta and "total" in meta:
                    ...
                    print(
                        f"   Progress: {progress:.1f}% ({meta['current']}/{meta['total']})"
                    )
                elif "status" in meta:
    ...
    def poll_with_webhook_simulation(
        self, task_id: str, max_attempts: int = 30
    ) -> Dict[str, Any]:
        """
        ...
        for attempt in range(max_attempts):
            ...
            if status["status"] == "completed":
                print("✅ Task completed! (Webhook would have been called)")
                return status
            elif status["status"] == "failed":
                print("❌ Task failed! (Webhook would have been called)")
                return status
