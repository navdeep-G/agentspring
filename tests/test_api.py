...
from agentspring.api import FastAPIAgent, AuthMiddleware
...
def unhealthy_app():
    ...
    with patch('agentspring.api.AsyncTaskManager') as MockAsyncTaskManager:
        ...
        mock_task_manager.celery_app.backend.client.ping.side_effect = Exception("Redis down")
...
def no_async_app():
    ...
    with patch('agentspring.api.AsyncTaskManager', side_effect=Exception("Redis not available")):
