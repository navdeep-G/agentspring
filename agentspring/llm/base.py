...
class ProviderConfig(BaseModel):
    ...
    validate_input: bool = Field(
        True, description="Validate input before processing"
    )
...
class RateLimitError(LLMError):
    ...
    def __init__(self, message: str, retry_after: int = 60, **kwargs):
        super().__init__(
            message, status_code=429, retry_after=retry_after, **kwargs
        )
...
class LLMProvider(ABC):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.logger = logging.getLogger(
            f"llm.provider.{self.__class__.__name__.lower()}"
        )
        ...
        self.logger.debug(
            ...
            extra={
                ...
                **{
                    ...
                    if not isinstance(v, (bytes, str)) or len(str(v)) < 100
                },
            },
        )
    ...
    def stream(self, prompt: str, **kwargs) -> Any:
            ...
            def __next__(self):
                if not self._task.done() and self._queue.empty():
                    ...
                    self._loop.run_until_complete(
                        asyncio.wait([self._queue.get()], timeout=30)
                    )
        ...
        try:
            ...
            if loop.is_running():
                ...
                new_loop = asyncio.new_event_loop()
                try:
                    async_gen = self._rate_limited_call(
                        self.stream_async, prompt, **kwargs
                    )
