...
class ToolRegistry:
    ...
    def register(
        ...
        def decorator(func: Callable):
            schema = self._generate_schema(
                func, name, description or func.__doc__ or ""
            )
    ...
    def _validate_and_map_parameters(
        self, tool: Callable, kwargs: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        for param_name, value in kwargs.items():
            ...
            if (
                tool_name in param_mappings
                and param_name in param_mappings[tool_name]
            ):
            ...
            if param_name in param_names:
                ...
                for actual_param in param_names:
                    if (
                        param_name.lower() in actual_param.lower()
                        or actual_param.lower() in param_name.lower()
                    ):
                    ...
                    logger.warning(
                        f"Unknown parameter '{param_name}' for tool '{tool_name}'. Available: {param_names}"
                    )
...
__all__ = [ToolRegistry, ToolSchema, ToolExecutionResult, tool, tool_registry]
