...
class BaseRequest(BaseModel):
    ...
    customer_id: str = Field(..., min_length=3, max_length=64, description="Customer identifier")
...
class TextRequest(BaseRequest):
    ...
    message: str = Field(..., min_length=5, max_length=2000, description="Text to process")
    
    @validator('message')
    def message_safe(cls, v):
        """Validate message doesn't contain unsafe content"""
        unsafe_patterns = ['<script', '</', '\\', 'eval(', 'import os', 'rm -rf']
...
class ClassificationResponse(BaseResponse):
    ...
    alternatives: Optional[List[str]] = Field(None, description="Alternative categories")

class SummarizationRequest(TextRequest):
    """Request model for summarization tasks"""
    max_length: Optional[int] = Field(100, ge=10, le=1000, description="Maximum summary length")
...
class PriorityRequest(TextRequest):
    ...
    priorities: Optional[List[str]] = Field(None, description="Available priority levels")
...
class PriorityResponse(BaseResponse):
    ...
    confidence: float = Field(..., ge=0.0, le=1.0, description="Priority confidence")
...
class SentimentResponse(BaseResponse):
    ...
    confidence: Optional[float] = Field(None, ge=0.0, le=1.0, description="Sentiment confidence")
